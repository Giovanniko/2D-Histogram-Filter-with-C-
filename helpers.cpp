...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	vector < vector <float> > factors {{cor_prob, adj_prob, cor_prob},
                           			   {adj_prob, cen_prob, adj_prob},
                           			   {cor_prob,adj_prob, cor_prob}};//Got ideas from://https://www.cpp.edu/~elab/ECE114/Array.html

	for (int row = 0; row < height; row++) {
    	for (int col =0; col < width; col++) {
        	float grid_val = grid[row][col];
          	
          	for (int dx = -1; dx < 2; dx++) {//creates indices for a 3x3 matrix around the current grid index
            	for (int dy = -1; dy < 2; dy++) {
                	float blur_factor = factors[dx+1][dy+1];
 
                  	int new_i = abs((row + dy) % height);
                  	int new_j = abs((col + dx) % width);
                  	newGrid[new_i][new_j] += blur_factor * grid_val;
                }
            }
        }
    }
...
// void printGrid( vector < vector < float > > grid, int height, int width) {
