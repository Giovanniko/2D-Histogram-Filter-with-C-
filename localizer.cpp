...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
	...
	int area = height * width;
      ...
      for (int col = 0; col < width; col++) {
      	newGrid[row][col] = beliefs_per_cell;
      }
    }
  ...
  float blurring) 
{
  ...
  for (int row = 0; row < height; row++) {
  	for (int col = 0; col < width; col++) {
    	int new_i = abs(row + dy) % width;
      	int new_j = abs(col + dx) % height;
	...
	float p_miss) 
{
	int height = grid.size();
  	...
  	for (int row = 0; row < height; row++) {
    	for (int col = 0; col < width; col++) {
        	float new_value;
          	float current_value = grid[row][col];
          	if (color == current_value) {
            	new_value = p_hit * beliefs[row][col];
              	newGrid[row][col] = new_value;
            } else {
            	new_value = p_miss * beliefs[row][col];
              	newGrid[row][col] = new_value;
            }
        }
    }
...
//Simulation simulation (newGrid, p_hit, blurring, pose)